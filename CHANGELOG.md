...
- **我的页卡（mine）布局对齐设置页**：采用与设置页同一套微信风格分组列表原语（`.group` 标题带 + 全宽平铺无圆角 `.group-card` + `.cell`），并修复此前深色模式不跟随（仅挂 `fontClass` 未挂 `darkClass`）；cell 为「标题 + 右 chevron」结构，无左图标、无头像资料卡，与设置页视觉完全一致
