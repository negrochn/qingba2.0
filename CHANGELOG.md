...
> 多孩家庭支持：每个孩子拥有独立的打卡记录、自定义资源、读次数、路线/阶段进度与目标口径，全 App 视图随「当前孩子」切换。老用户数据自动迁移至主孩子名下（幂等），单孩家庭零感知。
...
| 版本 | 说明 |
| ---- | ---- |
| 4.1.0 | 多孩家庭支持：每孩子独立的打卡/资源/阶段/目标数据（老用户自动迁移、单孩零感知）；首页纯头像切换器与切换弹层点行即切；新增孩子管理页（切换/改名/删除/添加）；备份 v3.0 多孩格式整体恢复、v2 旧文件先选目标孩子；修复改名弹窗拼音打不完。 |
