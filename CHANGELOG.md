...
> 多孩：每孩子独立打卡/资源/进度数据，老用户自动迁移，单孩零感知；首页头像/弹层点行即切；孩子管理页：切换/改名/删除/添加；备份v3.0整体恢复，v2先选目标孩子；修复改名弹窗拼音打不完。

### 新增

- **孩子数据层（`utils/children.js`）**：孩子列表 CRUD + 当前孩子 `activeChildId()`；孩子实体 `{ id, name, color, createdAt }`，头像色板五色（green/blue/orange/purple/pink）按添加顺序循环取色，软上限 5 个
  - **`per-child` 存储键全量叠加**（`utils/checkin.js` / `utils/customResources.js`）：打卡分块、读次数、路线/阶段/完成名单、目标档位与逐阶段自定义目标、自定义资源、有趣计划等键均带 `<childId>` 段；默认备注 / 熏听开关 / 字号等保持家庭级。页面层零改动——各页 `onShow` 重读即新孩子视图
  - **老用户平滑迁移**：首次启动自动创建主孩子（默认名「宝宝」）并打迁移标记；legacy 无 childId 的旧键在主孩子首次读取时一次性迁入新槽位并删除（幂等，兜底读双保险）；新添加的孩子从空数据开始
- **切换孩子弹层（`components/child-picker`）**：half-sheet 外壳 + `form-card` 卡片行列表（与目标时长弹窗同质感），每行头像 + 昵称 + 摘要（今日 N 次 · 累计 N 小时），当前项品牌绿对勾；**点行即切 + toast「已切换到 xx」**——切换是轻操作，不设确认步骤；首页 / 路线页共用
- **孩子管理页（`pages/childManage`）**：设置页「孩子管理」入口**常驻**（单孩也显示——它是添加第二个孩子的唯一路径，也是单孩改名的入口）
  - radio 列表点击行即切换当前孩子（绿勾移动 + toast），与切换弹层同语义
  - 左滑操作：左「改名」灰 / 右「删除」红（唯一孩子删除禁用）
  - 删除走**强确认**：弹层列出后果（记录 / 资源 / 进度全清、不可恢复），输入孩子昵称一致才可删；删除的是当前孩子时自动回退剩余第一个
  - 底部「添加孩子」：输入昵称创建后自动切换为新孩子，全 App 多孩 UI 随 `children.length > 1` 出现
- **备份格式升级 v3.0 多孩（`pages/settings`）**：导出含每个孩子全部数据（打卡扁平数组 / 读次数 / 自定义资源 / 两路线阶段槽位 / 完成名单 / 当前路线 / 目标口径）；导入 v3 文件按名字匹配逐孩整体恢复（满 5 跳过），**v2 旧文件先弹选孩子弹层**再导入——临时切换执行、`try/finally` 保证当前孩子复位，旧格式天然兼容

### 变更

- **首页右上角孩子切换器（`pages/home`）**：仅多孩显示，形态为**纯头像**（`cp-av` 72rpx 标准档，外层热区补足 88rpx、负 margin 保持右对齐）——归属由头像颜色承载，名字与对勾在切换弹层内核对；同时 header 的「阶段名 + 进度」块并入统计卡（全局改版，单孩同样生效，避免维护两套首页布局）
- **路线页顶部「当前孩子」cell（`pages/route`）**：多孩时 cells 首行插入（名字 + 摘要 + 箭头，点击弹切换弹层），「为谁学 → 学到哪 → 怎么走」层级递进；「当前阶段」分组标题移除（单行分组本冗余，全局改版），顶部留白由容器 `padding-top` 补齐
- **改名 / 添加 / 删除弹窗对齐全局弹层表单原语（`pages/childManage` / `pages/myResources`）**：`form-card` + `form-row` 卡片质感（与打卡弹窗同款）；label「昵称」，字数限制收进 placeholder（「最多8个汉字或16个字母」），不占独立说明行
- **昵称按「显示宽度」计数（`utils/children.js`）**：汉字计 1、字母 / 数字计 0.5（最多 8 个汉字或 16 个字母）——纯汉字口径会把 Christopher 这类英文名挡在门外；`clipName()` 失焦截断与 `_validateName()` 提交校验同口径
- **`cp-av` 头像原语统一 72rpx（`app.wxss`）**：路线页 / 管理页 / 切换弹层 / 首页同档；移除原首页胶囊专用的 48rpx `--sm` 死档
- **移除已落地的设计文档 `design/multi-child-design.md`**：多孩功能已实现，通用 UI 规范沉淀进 `ui-design-spec` skill（弹层输入框 maxlength 拼音坑、小可点元素热区补足 88rpx 等），7 处代码注释中的文档引用同步清理
- **清理死代码 `checkin.clearAllCheckins`**：多孩改造后清空功能统一走按阶段的 `clearCheckinsByStage`，该「清当前孩子全部打卡（含分片 / legacy）」函数已零引用，连同导出一并删除

### 修复

- **改名 / 资源弹窗拼音打不完（`pages/childManage` / `pages/myResources`）**：小程序 `input` 的 `maxlength` 在拼音输入法**组合期间把字母也计入长度**——昵称限 8 字时全拼 `xiaoerzi` 打满 8 个字母就被硬截，汉字没机会上屏。放开为 `-1`，失焦（组合必已结束）按显示宽度截断 + toast 提示，提交时校验兜底；输入全程不打断拼音
- **tabBar 页弹窗切 tab 后残留（`pages/home` / `pages/route`）**：原生 tabBar 层级高于弹层蒙层，弹窗开着也能直接点「路线 / 我的」切走；切 tab 只 hide 不销毁页面，回来蒙层还挂着，且孩子列表可能已在「我的」里被增删后过期。两页 `onHide` 兜底收起弹层（首页含今日明细弹窗；滑出动画在页面隐藏期间照常走完，回来已是干净状态），并移除 `onShow` 里「发现弹窗开着就重建内容」的死分支

## [4.0.0] - 2026-09-23

> 多路线架构落地：新增「大循环路线」（9 个阶段，含 3 条可选调整支线），路线可切换、数据按路线隔离；新增路线**毕业态**——走完主链终点后首页切换为毕业庆祝卡；阶段完成判定由「数组序」升级为「晋级链图语义」；压力测试按各阶段官方时长目标生成。
> 可用 `git log v3.7.2..v4.0.0` 查看对应提交范围。

### 新增

- **大循环路线与多路线架构（`utils/bigloop_route.js` / `utils/data.js` / `utils/checkin.js` / `pages/routePicker`）**：接入官方「大循环路线」——9 个阶段（前置、一阶段、3 条可选调整支线「调整·牛1-3」、标准·牛4-6、三阶段牛7-9），与常规路线经 `ROUTES` 注册表统一（`{ regular, bigloop }`，`id` / `name` / `stages` / `journeyText` 同构）。
  - **数据按路线隔离**：当前阶段、打卡记录、完成名单等存储键带路线后缀分槽（`_currentStageStorageKey()` 等），两条路线互不干扰；非法路线 id 读取时回退 `regular`
  - **路线切换页 `pages/routePicker`**（设置页入口）：整页单选，切换时 toast 说明「数据按路线隔离」；阶段选择、目标时长、资源分组、统计、备份清空等全链路按当前路线渲染
  - **可选支线（`optional: true`）语义**：调整支线无时长目标、不占主线晋级链（prev 全指向一阶段、无后继）——切入级别看测试结果，由家长在阶段选择页自行切换；路线页显示「可选支线」徽标、进度显示「—」而非 0%
  - 原始数据 `qingba_big_loop_route.json` 入库（399 行官方文档拍平前源）；大循环晋级链显式声明 `prev_stage_ids`（分叉结构）
- **路线毕业态（`utils/data.js` 的 `isRouteCompleted()` / `getFinalStage()` + `pages/home` 毕业卡）**：当前阶段是主链终点（数组中最后一个非 optional 阶段：常规=准桥梁，大循环=三阶段）且已被显式标记完成时，首页切换为**毕业庆祝卡**——
  - **判定是纯运行时计算，零存储迁移**：历史上点过「完成阶段」的老用户自动生效；经阶段选择页切走（如回头复习）后判定自动失效、回到普通形态
  - 毕业卡 = 品牌绿徽章白勾 + confetti 纯色散点 + 按路线区分的庆祝标题（「常规路线圆满完成」/「大循环路线圆满收官」）+ 副题 + **全程口径三联数字**（天陪伴 / 小时累计 / 天打卡，去掉阶段过滤遍历全部记录）；顶部问候切「Congratulations, Day N」（N = 全程陪伴天数）
  - **毕业 ≠ 停止记录**：今日/本周数据与打卡入口保留（自由听读仍落当前阶段锚点）；进行态统计卡片不渲染；顶部右侧「阶段名 + 百分比」整块隐藏（祝贺语与毕业卡已承担全部语义，不再暴露内部阶段口径）
- **大循环路线介绍页（`pages/about-bigloop`）**：路线介绍 / 阶段参考 / 目标与达标判断 / 使用说明静态成文，「逐阶段素材与要点」按官方《大循环素材汇总2.0》整理为页面内 `DOC_STAGES`，与路线数据层解耦；样式 `@import` 复用 `about.wxss`，仅补「可选支线」文字徽标一个原语
- **阶段页关键要点折叠（`pages/stage`）**：要点 ≤3 条全展开无按钮；更多时默认只显前 2 条，底部渐隐 + 「展开全部 N 条 / 收起」品牌绿文字按钮（箭头用 `icon-right` 旋转）——大循环前置阶段 8 条长要点不再把打卡主流程挤到很深；要点区同时由逐条 `weui-cell` 改为整块连续段落卡（轻量文字序号分条）
- **可选支线页标识（`pages/stage`）**：进入未走过的调整支线时，分组标题旁显示「可选支线」浅底胶囊徽标（`--card2` 底 + `--text3` 字，同路线页 `.route-badge` 视觉语言）

### 变更

- **阶段完成判定升级为「晋级链图语义」（`utils/data.js` 的 `isUpstreamStage()`）**：沿 `prev_stage_ids` 反向回溯（frontier + seen 防环）判定「是否为当前阶段的前序」，替代原「数组序在前」的推断；调用方为路线页 done 态与阶段页 completed 态。链式路线（常规）行为零变化；分叉路线（大循环）下调整支线虽在数组序里靠前，但不在主线回溯链上，**走到标准牛4-6 时支线不再被误判为已完成**。阶段页原先两处重复的「索引三元」判定收敛为 `_calcStageStatus()` 一处
- **下一阶段查找主线优先（`pages/stage` 的 `findNextStage()`）**：后继候选中**跳过 optional 支线**再取主线（原先「数组序第一个后继」在大循环下会让一阶段误指向调整支线）；若后继全是可选支线则返回 null 走「完成阶段」，宁可交还家长也不自动误入
- **无时长目标的阶段不显示 0% 假进度（`pages/route` / `pages/home`）**：调整支线等无目标阶段（`time_investment` 为空）进度显示「—」占位；首页顶部同口径
- **「我的」页路线介绍入口按当前路线二选一（`pages/mine`）**：只显示当前路线的介绍入口（与全 app「只服务当前路线」口径一致，想了解另一条走路线选择页）；`onShow` 刷新，切路线返回即更新；文案保留具体名（「常规路线介绍」/「大循环路线介绍」）与目标页标题一致
- **压力测试按各阶段官方时长目标生成（`utils/stress-test.js`）**：新增 `getStageTargetMinutes()` 复用 `parseTargetHours`——区间目标（60-80H）在区间内随机、单值（60H/90H）取固定值、80H+ 取下限；**无官方目标的阶段（大循环调整支线）按「试走量」12-20 小时生成**，模拟「测试调整策略数日后回主线」，阶段保持 optional 未完成、打卡记录保留；解析失败回退原 80-90h。确认弹窗文案同步
- **关键要点行内强调改亮度台阶 + 700（`pages/stage/stage.wxss`）**：`kp-text-hi` 由 `--text2`+600 改 `--text`+700——600 在 Android（Roboto 只有 400/500/700）会被就近映射成 medium、真机上加粗几乎看不出，亮度台阶同时兜底字重失真（与 3.7.2「关于」页同款结论）
- **设置页「学习设置」拆为「当前进度」+「学习设置」（`pages/settings`）**：原先 6 项挤在一组，语义混杂——「当前路线 / 当前阶段」是状态项（我现在学到哪），与「阶段目标时长 / 我的资源 / 两个开关」这类偏好配置（时长怎么算）不是一回事；拆组后前者置顶为「当前进度」，后者保留「学习设置」名称。纯分组展示调整，跳转与数据逻辑零改动
- **移除已落地的方案文档 `docs/大循环路线接入方案.md`**：多路线架构已实现，方案文档完成使命

### 修复

- **压力测试把最后阶段误标为已完成（`utils/stress-test.js`）**：链式回溯 `walk` 抄自 stagePicker 却漏掉关键一行——`delete chain[lastStage.stage_id]`（stagePicker 注释明确「名单不含当前阶段自身」）。于是压测后完成名单含最后阶段 → 详情页 `alreadyDone` 只读、「完成阶段」按钮不渲染，看起来像「已完成」实为误标。修复后最后阶段保持 `current`（可打卡、可点「完成阶段」），常规路线的准桥梁同样受益
...
| 版本 | 说明 |
| ---- | ---- |
| 4.1.0 | 多孩：每孩子独立打卡/资源/进度数据，老用户自动迁移，单孩零感知；首页头像/弹层点行即切；孩子管理页：切换/改名/删除/添加；备份v3.0整体恢复，v2先选目标孩子；修复改名弹窗拼音打不完。 |
| 4.0.0 | 多路线架构落地：新增大循环路线（9 阶段含 3 条可选调整支线），路线可切换、数据按路线隔离；新增路线毕业态（首页毕业庆祝卡）；阶段完成判定改晋级链图语义；压力测试按各阶段官方时长目标生成。 |
