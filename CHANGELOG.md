...
- **移除「启蒙年龄」功能**：首启引导、晋级年龄填写弹窗、设置页「孩子信息」分组，以及按阶段年龄存储（`qingba_stage_ages`）全部删除；当前阶段引导与晋级流程不再涉及年龄记录
- 统计展示不再依赖启蒙年龄；备份导入/导出与清空数据不再包含启蒙年龄相关字段
...
- 清空全部数据时清理历史残留的 `qingba_stage_ages` key，避免废弃数据长期留存
...
| 版本 | 说明 |
| ---- | ---- |
| 2.3.0 | 新增首次启动引导（设启蒙年龄与当前阶段，前序自动标记完成）；启蒙年龄按阶段记录（首启设首阶段、晋级填下一阶段）；设置页只读展示；备份导入/清空兼容完成名单与未设置态。 |
