...
- **打卡弹窗（stage）按 skill 重构 + 再次优化**：从私有 `.ci-*` 类切换为原语 4 `.sheet-mask`/`.action-sheet`、原语 13/15 `.form-card`/`.form-row`/`.form-field`、原语 14 `.seg-control`/`.seg-item`（10/15/20/30 快捷时长，选中态走 `brand #07C160`）、原语 10 变体 `.badge-pill-brand`（"读完" 绿 pill）；底部按钮再次优化为 **skill 原语 16 标准 `.btn` 族**（`.btn-row` 居中并排布局 + `.btn-secondary` 取消 + `.btn-primary` 打卡，作用域 `.action-sheet .btn*` 限定避免与 `app.wxss` 全局 `.btn-primary` 冲突）；取消按钮字色灰改黑（更微信）、主按钮去蓝色渐变改实心 `var(--brand)` 与 seg-active 一致；移除 `.dm-dark`/`.dm-auto` 中已废弃的 `.ci-*` 选择器；新增 skill 原语 `.badge-pill-brand`（品牌完成态 pill）
