...
> 接入：根节点加 `data-weui-mode='care'`（如微信「关怀模式」）；其余 token 同名自动切换。**本项目不启用 WeUI 的 care 档**——适老化改走「字号跟随微信字体设置」（官方《小程序适老化设计指南》推荐的正是后者，见 `SKILL.md`「项目实现备注」）；若将来要上 care 配色，再按本节机制加 `care-light` / `care-dark` 变量档。

---

## 9. 项目变量映射（qingba 自有 `--*`，已与 WeUI 对齐）

本项目 `app.wxss` 用根节点 `.dm-auto`（写在 `@media (prefers-color-scheme: dark)` 内）切换自有变量 —— 早期 `dm-light` / `dm-dark` 两个手动档已移除，规则体保留为休眠机制；其取值已对齐 WeUI，可直接当作 WeUI token 的别名使用：

| 项目变量 | 值（浅 / 深） | 对应 WeUI |
|---|---|---|
| `--bg` | `#ededed` / `#111111` | BG-0 |
| `--card` | `#ffffff` / `#191919` | BG-2 |
| `--card2` | `#f7f7f7` / `#202020` | ≈ BG-3（三级背景 / 卡片内嵌底、次级填充） |
| `--text` | `#1a1a1a` / `rgba(255,255,255,.8)` | FG-0（主文字） |
| `--text2` | `#737373` / `rgba(255,255,255,.5)` | FG-1（标题 / 说明 / cell 值） |
| `--text3` | `#b2b2b2` / `rgba(255,255,255,.3)` | FG-2（三级 / 占位） |
| `--divider` | `#e5e5e5` / `rgba(255,255,255,.1)` | ≈ FG-3（.1 黑≈#e6e6e6） |
| `--brand` | `#07c160` | BRAND |
| `--danger` | `#fa5151` | RED（深浅一致，不随主题变化） |
| `--cell-active` | `#ececec` / `#373737` | ≈ BG-COLOR-ACTIVE（按下态底） |
| `--btn-default` | `rgba(0,0,0,.05)` / `rgba(255,255,255,.1)` | **FG-5（默认按钮底）** |

> 浅色档的 `--text` / `--text2` / `--text3` / `--divider` 是 WeUI 半透明 FG 的**不透明近似值**（深色档直接沿用 FG 的 rgba）。
> **`--btn-default` 两端都必须保持半透明**：同一枚 `.weui-btn_default` 既要落在 `--bg`（页面，如 `stage` 的晋级按钮）又要落在 `--card`（弹窗，如「取消」）上——换成任何不透明实色，都必然在另一种底上隐形（历史两次踩坑：`--cell-active` 撞 `--bg`、`--card` 撞 `--card`）。
> **`--cell-active` 的「一档」是按 `--card` 底选的**：`#ececec` / `#373737` 与 `--card`（`#ffffff` / `#191919`）差约 19 / 23 个色阶，因此**透明的**元素（如 `.form-row`）一旦落在 `--card2` 上，按压反馈要改取 `--divider`（浅 `#e5e5e5` → 差 18；深 `rgba(255,255,255,.1)` 叠在 `#202020` 上 ≈ `#363636` → 差 22），沿用 `--cell-active` 只剩 11 个色阶、按住几乎看不出。**自带底色的元素不受影响**（如 `.seg-item` 是「白 → `#ececec`」，跟容器底无关，照用 `--cell-active`）。判据与上一条同源：跨越不同底的交互色，按最弱的那个底来选。

> 文字档已收敛为三档 `--text` / `--text2` / `--text3`，一一对应 WeUI FG-0 / FG-1 / FG-2；早期存在的 `--text4`（与 `--text3` 同值）已合并移除，新代码不要再引用。

### 项目字号档

| rpx | px | 定位 | 典型用法 |
|---|---|---|---|
| 44 | 22 | 文章标题 / 空状态主标题 | 关于页文章主标题（`about` 的 `.weui-article__h1`）、欢迎卡主标题（`.home-welcome__title`，对齐 WeUI `.weui-msg__title` 22px）；对应设计指南 22pt 档 |
| 40 | 20 | 文章章节标题 | **仅**关于页 `h2`（`.weui-article__h2`）——WeUI 官方 article 的 h2 就是 20px，**比正文大一档**才撑得起章节层级（见 `components.md` 原语 21）。注意与统计页「大号数字」的 40 / 44rpx 同值不同语境，勿混用 |
| 34 | 17 | 主文 | `.weui-cell__bd_text`、按钮、dialog 标题 / 正文、**文章页正文、h3·h4 与列表项**（WeUI article 正文基准即 17px / FG-0） |
| 30 | 15 | 长文说明 | `.weui-panel__hd`、空状态说明（`.home-welcome__desc`）、记录 / 统计页说明文案；**仅限长文说明类**，标题、值、按钮一律不用 |
| 28 | 14 | 次级 | 分组标题、说明、cell 右侧值、关键要点文字 |
| 24 | 12 | 辅助 | `.weui-cell__desc`、徽标、箭头、角标 |

- 统一写作 `calc(Nrpx * var(--fs, 1))`，随字体档位（小 / 标准 / 大 / 特大）缩放；**档位由微信字体设置映射而来**（`fontSizeScaleFactor` → 四档，见 `SKILL.md`「项目实现备注」），不是页内手动选择
- **标准 UI 元素**（列表标题 / 值 / 按钮 / 标签 / 卡片内文案）只用 34 / 28 / 24 三档；`40rpx`(20px) 目前仅文章页章节标题（原语 21）；`30rpx`(15px) 仅用于长文说明 / 面板标题 / 空状态
- **长文页档位重定（3.4.1）**：关于页此前用 `30rpx`(15pt) + `--text2`，已改回**正文 `34rpx`(17pt) + `--text`**，并把「3-4岁 · 常规路径」提为 `44rpx`(22pt) 文章主标题——15pt 在微信档位里的定位是「过渡次级字号」，用在文章正文偏小偏灰。层级模型与列表语义见 `components.md` 原语 21
- **文章页照官方 article 数值（3.7.2）**：关于页新增 `40rpx`(20px) 章节档（官方 h2 就是 20px，比正文大一档），并把 h1 改**居中 + 下距 96rpx**、h4 回到 **400**（官方不加粗）、正文行高 1.6、**段落下距 `48rpx`**(官方 24px)、列表缩进 40rpx、容器 padding `96rpx 48rpx`。**层级靠字号差与留白，不靠字重与边界元素** —— 此前试过「h2 加绿竖条 / 章节间细线 / 卡片化」，深色下收益低且观感碎，已否掉
- **文章页档位已 token 化（3.7.2）**：`about.wxss` 用 `--art-*` 承载「字号 / 行高 / 字重」三件套（`--art-h1…h4` / `--art-body` / `--art-note` 及各自的 `-lh`、`-w`）与间距阶梯（`--art-gap-para/block/section` = 48 / 64 / 96rpx），元素只引 token 名；行高改为**与字号成对写死的 rpx**（不再是 1.6 倍率，仍乘 `--fs`）。做法借自 TDesign Typography，**数值不借** —— TDesign 默认正文仅 14px、标题一律 `600`、h4 档(28rpx)比正文还小，属「App 界面层级」而非「长文阅读层级」
- **已废弃**：`26rpx`（13px，非 WeUI 档）与按比例放大的 `29 / 32 / 36 / 41rpx`，全站已清零，新代码不要再使用
- 大号数字（40 / 44 / 56 / 72rpx）用于统计与时长展示，不受上表约束（`40rpx` 与文章章节档同值、语境不同，勿混用）

**约定**：新代码优先用项目 `--*` 变量（已对齐 WeUI），不要硬编码色值；如确需引用 WeUI 原始变量名，使用上表 `--weui-*` 值。标准 UI 元素字号只用 34 / 28 / 24 三档（长文正文可用 30rpx，数据展示可用大号数字）。
