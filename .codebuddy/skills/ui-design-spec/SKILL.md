...
---
## TODO：待补充
- 截图未覆盖的组件：居中 modal（UIAlert）、空状态、加载 / 骨架屏、滚轮选择器（地区 / 性别 / 日期）、滑块（字号实时预览）、chip 多选、消息详情页、强提醒 / 免打扰 / @全员 等群消息角标（需更多截图）。表单输入已通过原语 13 `.form-page` / 原语 14 `.seg-control` / 原语 15 `.form-input` 覆盖。
- caption 超过 2 行的处理约定。
- 字号系统缩放方案（是否引入 `--fs`）。
- 图标规范（功能图标尺寸 / 风格）。
